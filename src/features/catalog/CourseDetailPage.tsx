...
export function CourseDetailPage() {
  ...
  const canManageCourses =
    user?.roles.some(
      (role) => role === 'INSTRUCTOR' || role === 'LECTURE' || role === 'ADMIN',
    ) ?? false
  ...
  if (!course) {
    return (
      <main className="grid min-h-[60vh] place-items-center text-slate-500">
        Đang tải khóa học...
      </main>
    )
  }
      ...
      <section className="bg-ink text-white">
        <div className="mx-auto grid max-w-6xl gap-10 px-5 py-16 lg:grid-cols-[1fr_340px] lg:items-center">
          <div>
            ...
            <p className="mt-8 text-sm font-semibold uppercase tracking-[0.2em] text-brand-100">
            ...
            <div className="mt-7 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-300">
              ...
              <span>
                {formatCourseDuration(course.estimatedDurationMinutes)}
              </span>
          ...
          <aside className="rounded-2xl bg-white p-7 text-ink shadow-2xl">
            ...
            <p className="mt-4 text-center text-xs text-slate-500">
              Ghi danh miễn phí ngay; khóa trả phí cần hoàn tất checkout.
            </p>
          </aside>
        </div>
      </section>
