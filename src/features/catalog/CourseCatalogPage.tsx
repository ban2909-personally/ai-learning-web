    ...
    <main>
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-6xl px-5 py-14">
          ...
          <form
            onSubmit={submit}
            className="mt-8 grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-[1fr_220px_180px_auto]"
          >
            ...
            <button className="rounded-xl bg-ink px-6 py-3 font-semibold text-white hover:bg-slate-700">
              Tìm kiếm
            </button>
          </form>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-5 py-12">
        {!isLoading && !error && (
          <p className="mb-6 text-sm text-slate-500">
            {result?.totalElements ?? 0} khóa học phù hợp
          </p>
        )}
        {isLoading && (
