...
export function LessonMediaManagerPage() {
  ...
  const canManage =
    user?.roles.some((role) =>
      ['INSTRUCTOR', 'LECTURE', 'ADMIN'].includes(role),
    ) ?? false

  useEffect(() => {
    if (!slug || !canManage) return
    apiRequest<CourseCurriculum>(
      ...
      .catch((caught: unknown) =>
        setPageError(
          caught instanceof ApiError
            ? caught.message
            : 'Không thể tải giáo trình khóa học.',
        ),
      )
  }, [canManage, slug])
  ...
  const submit = async (lessonId: string) => {
    ...
    if (validation) {
      updateState(lessonId, { error: validation })
      return
    }
    ...
    <main className="mx-auto max-w-5xl px-5 py-10 sm:py-14">
      ...
      <div className="mt-7 max-w-3xl">
        ...
        <p className="mt-4 leading-7 text-slate-600">
          Tải video MP4 hoặc WebM, nhỏ hơn 10 MB. Video mới chỉ được công bố cho
          học viên có quyền truy cập bài học.
        </p>
      </div>
      ...
      {!curriculum && !pageError && (
        <p className="mt-10 text-slate-500">Đang tải danh sách bài học...</p>
      )}
                    ...
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          {lesson.preview
                            ? 'Bài học xem trước'
                            : 'Chỉ dành cho học viên đã ghi danh'}
                        </p>
                      </div>
